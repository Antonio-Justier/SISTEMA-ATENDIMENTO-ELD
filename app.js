...
let CU=null,PRODUCTS=[],ALLPRODUCTS=[],CATEGORIES=[],UMAP={},CP='';
/* ─── MULTI-ESTOQUE ─── */
...
function visibleLocs(){if(isAdmin())return LOCATIONS.filter(l=>l.active!==false);if(isLeader())return LOCATIONS.filter(l=>MYLEADS.includes(l.id));return LOCATIONS.filter(l=>MYLOCS.includes(l.id));}
...
async function loadProds(){
  ...
  ALLPRODUCTS=(prods||[]).map(p=>({...p,category:p.categories?.name||'Geral'}));
  PRODUCTS=ALLPRODUCTS.filter(p=>(p.approval_status||'approved')==='approved'); // catálogo requisitável: só aprovados
  const {data:us}=await sb.from('users').select('id,full_name');
  UMAP=Object.fromEntries((us||[]).map(u=>[u.id,u.full_name]));
}
...
async function renderItens(c){
  c.innerHTML='<div class="loading"><i class="ti ti-loader-2"></i>Carregando...</div>';
  await loadProds();
  const base=cadProducts();
  const cats=['Todos',...new Set(base.map(p=>p.category).filter(Boolean).sort())];
  const pend=isAdmin()?ALLPRODUCTS.filter(p=>(p.approval_status||'approved')==='pending'):[];
  const pendHTML=pend.length?`<div class="tcard" style="margin-bottom:14px"><div class="tcard-hd"><h3><i class="ti ti-clock"></i> Itens aguardando aprovação <span class="tag-adm">${pend.length}</span></h3></div>
  <div style="display:flex;flex-direction:column;gap:8px">${pend.map(p=>{const brk=LOCATIONS.map(l=>stockOf(l.id,p.id)>0?`${esc(l.name)} ${stockOf(l.id,p.id)}`:null).filter(Boolean).join(' · ')||'sem estoque';return `<div style="display:flex;align-items:center;gap:10px;padding:9px 12px;border:1.5px solid var(--border);border-radius:10px;flex-wrap:wrap"><div style="flex:1;min-width:160px"><div style="font-weight:600;font-size:13px">${esc(p.name)}${p.serial_code?` <span style="font-family:'DM Mono',monospace;font-size:10px;color:var(--red)">[${esc(p.serial_code)}]</span>`:''}</div><div style="font-size:11px;color:var(--muted)">${esc(p.category)} · por <b>${esc(uName(p.created_by))}</b> · ${brk}</div></div><button class="btn-red" style="font-size:12px;padding:6px 12px" onclick="approveProd('${p.id}')"><i class="ti ti-check"></i> Aprovar</button><button class="btn-out" style="font-size:12px;padding:6px 12px" onclick="rejectProd('${p.id}','${esc(p.name).replace(/'/g,"\\\'")}')"><i class="ti ti-x"></i> Recusar</button></div>`;}).join('')}</div></div>`:'';
  c.innerHTML=`${pendHTML}<div style="display:flex;gap:10px;margin-bottom:12px;align-items:center;flex-wrap:wrap"><input type="text" class="srch" style="width:220px" id="isrch" placeholder="Buscar por nome ou N° série..." oninput="renderIG()"><span style="font-size:12px;color:var(--muted)">${base.length} ${isAdmin()?'itens no catálogo':'itens cadastrados por você'}</span></div>
...
function prodApprovalBadge(p){const st=(p.approval_status||'approved');
  if(st==='pending')return '<div style="margin-top:4px"><span class="badge b-pend">Aguardando aprovação</span></div>';
  if(st==='rejected')return '<div style="margin-top:4px"><span class="badge b-broken">Recusado — edite p/ reenviar</span></div>';
  if(p.approved_by)return `<div style="margin-top:4px;font-size:9px;color:var(--muted)">✓ Aprovado por ${esc(uName(p.approved_by))}</div>`;
  return '';}
function renderIG(){
  const g=$('ig');if(!g)return;
  const f=($('isrch')?.value||'').toLowerCase();
  const list=cadProducts().filter(p=>(_catF==='Todos'||p.category===_catF)&&(p.name.toLowerCase().includes(f)||(p.serial_code&&p.serial_code.toLowerCase().includes(f))));
...
async function saveItm(prodId){
  ...
  try{
    ...
    if(prodId&&prodId!=='null'){
      const cur=ALLPRODUCTS.find(x=>x.id===prodId);
      // líder só edita itens que ele cadastrou
      if(!isAdmin()&&cur&&cur.created_by!==CU.id){toast('Você só pode editar itens que cadastrou.','err');return;}
      // líder reenvia item recusado -> volta para pendente
      if(!isAdmin()&&cur&&(cur.approval_status||'approved')==='rejected'){payload.approval_status='pending';}
      const {error}=await sb.from('products').update(payload).eq('id',prodId);
      if(error)throw new Error(error.message);
      toast(payload.approval_status==='pending'?'Reenviado para aprovação.':'Atualizado!','ok');
    }else{
      // admin cadastra já aprovado; líder cadastra como pendente (precisa de aprovação do admin)
      payload.created_by=CU.id;
      if(isAdmin()){payload.approval_status='approved';payload.approved_by=CU.id;payload.approved_at=new Date().toISOString();}
      else{payload.approval_status='pending';}
      const {data:np,error}=await sb.from('products').insert(payload).select().single();
      if(error)throw new Error(error.message);
      tid=np?.id;
      toast(isAdmin()?'Cadastrado!':'Enviado para aprovação do admin.','ok');
    }
...
async function approveProd(id){
  ...
  const {error}=await sb.from('products').update({approval_status:'approved',approved_by:CU.id,approved_at:new Date().toISOString()}).eq('id',id);
  if(error){toast('Erro: '+error.message,'err');return;}
  toast('Item aprovado'+(p?': '+p.name:'')+'.','ok');
  await loadProds();await loadStock();renderItens($('content'));
}
async function rejectProd(id,name){
  if(!confirm('Recusar "'+(name||'item')+'"? O item não entra no catálogo. Quem cadastrou poderá editar e reenviar.'))return;
  const {error}=await sb.from('products').update({approval_status:'rejected'}).eq('id',id);
  if(error){toast('Erro: '+error.message,'err');return;}
  toast('Item recusado.','ok');
  await loadProds();renderItens($('content'));
}
